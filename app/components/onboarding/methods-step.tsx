interface MethodsStepProps {
  ...
  onChange: (value: MethodsStepProps['value']) => void;
}

export function MethodsStep({ value, onChange }: MethodsStepProps) {
  function addMethod() {
  ...
  function updateMethod(index: number, label: string) {
    onChange(value.map((m, i) => (i === index ? { label } : m)));
  }
    ...
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          How do you pay for things? Add your usual methods.
        </p>
      </div>
        ...
        {value.map((method, index) => (
          ...
          >
            <input
              ...
              className="min-w-0 flex-1 rounded-lg border-2 border-slate-200 px-3 py-2.5 text-sm text-slate-700 outline-none placeholder:text-slate-400 focus:border-slate-900"
            />
            <button
              type="button"
